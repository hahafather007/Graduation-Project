...
public class WavMergeUtil {
    ...
    private static boolean isFmt(byte[] bytes, int start) {
        return bytes[start + 0] == 'f' && bytes[start + 1] == 'm' && bytes[start + 2] == 't' && bytes[start + 3] == ' ';
    }

    private static boolean isData(byte[] bytes, int start) {
        return bytes[start + 0] == 'd' && bytes[start + 1] == 'a' && bytes[start + 2] == 't' && bytes[start + 3] == 'a';
    }
