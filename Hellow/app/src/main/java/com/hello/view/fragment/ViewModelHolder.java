...
public class ViewModelHolder<T extends ViewModel> extends Fragment {
    private static final String TAG = ViewModelHolder.class.toString();

    private T viewModel;

    public static <T extends ViewModel> T get(AppCompatActivity activity, Supplier<T> constructor) {
        return get(activity.getSupportFragmentManager(), constructor);
    }

    public static <T extends ViewModel> T get(Fragment fragment, Supplier<T> constructor) {
        return get(fragment.getChildFragmentManager(), constructor);
    }

    private static <T extends ViewModel> T get(FragmentManager fm, Supplier<T> constructor) {
